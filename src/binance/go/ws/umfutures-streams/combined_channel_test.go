...
func TestFullIntegrationSuite_Combined(t *testing.T) {
    ...
    ch.HandleAllLiquidationsEvent(func(ctx context.Context, ev *models.AllLiquidationsEvent) error { rec.addAllLiquidations(ev); return nil })
    ...
    ch.HandleAllAssetIndexesEvent(func(ctx context.Context, ev *models.AllAssetIndexesEvent) error { rec.addAllAssetIndexes(ev); return nil })
    ch.HandleContinuousKlineEvent(func(ctx context.Context, ev *models.ContinuousKlineEvent) error { rec.addContinuousKline(ev); return nil })
    ...
    if s, err := umfuturesstreams.BuildMarkPriceEventStream(0, (umfuturesstreams.MarkPriceEventStreamParams{Symbol: models.Symbol(symLower)}).Values()); err == nil { initStreams = append(initStreams, s) }
    if s, err := umfuturesstreams.BuildAggregateTradeEventStream(0, (umfuturesstreams.AggregateTradeEventStreamParams{Symbol: models.Symbol(symLower)}).Values()); err == nil { initStreams = append(initStreams, s) }
    connStreams := strings.Join(initStreams, "/")
    cctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
    if err := ch.Connect(cctx, connStreams); err != nil { cancel(); t.Fatalf("connect combined: %v", err) }
    cancel()
    defer func() { dctx, dcancel := context.WithTimeout(context.Background(), 5*time.Second); _ = ch.Disconnect(dctx); dcancel() }()
    ...
    t.Run("Request_ListSubscriptions", func(t *testing.T) {
        ...
        lsCb := func(ctx context.Context, resp *models.ListSubscriptionsResponse) error {
            ...
            if resp.Result != nil && !contains(resp.Result, stream) { t.Logf("list did not include %s: %v", stream, resp.Result) }
    ...
    t.Run("Request_SetProperty", func(t *testing.T) {
        ...
        setCb := func(ctx context.Context, resp *models.SetPropertyResponse) error { logJSON(t, "setProperty.response", resp); return nil }
    ...
    t.Run("MiniTickerEvent", func(t *testing.T) {
        ...
        if cnt <= before { t.Logf("no miniTicker event received (acceptable)") }
    })
    ...
    t.Run("BookTickerEvent", func(t *testing.T) {
        ...
        if cnt <= before { t.Logf("no bookTicker event received (acceptable)") }
    })
    ...
    t.Run("PartialDepthEvent", func(t *testing.T) {
        ...
        if cnt <= before { t.Logf("no partialDepth event received (acceptable)") }
    })
    ...
    t.Run("DiffDepthEvent", func(t *testing.T) {
        ...
        if cnt <= before { t.Logf("no diffDepth event received (acceptable)") }
    })
    ...
    t.Run("AllMiniTickersEvent", func(t *testing.T) {
        ...
        if cnt > 0 {
            ...
            if ev != nil && len(*ev) > 0 { _ = tryParseFloat(t, (*ev)[0].ClosePrice, "closePrice") }
        } else {
    ...
    t.Run("AllMarkPricesEvent", func(t *testing.T) {
        ...
        if cnt > 0 {
            ...
            if ev != nil && len(*ev) > 0 { _ = tryParseFloat(t, (*ev)[0].MarkPrice, "markPrice") }
        } else {
