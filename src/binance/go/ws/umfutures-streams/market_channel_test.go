...
func (r *umMarketEventRecorder) addAgg(ev *models.AggregateTradeEvent) {
	r.mu.Lock()
	r.agg = append(r.agg, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addMark(ev *models.MarkPriceEvent) {
	r.mu.Lock()
	r.mark = append(r.mark, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addKline(ev *models.KlineEvent) {
	r.mu.Lock()
	r.kline = append(r.kline, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addTicker(ev *models.TickerEvent) {
	r.mu.Lock()
	r.ticker = append(r.ticker, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addMiniTicker(ev *models.MiniTickerEvent) {
	r.mu.Lock()
	r.miniTicker = append(r.miniTicker, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addLiquidation(ev *models.LiquidationEvent) {
	r.mu.Lock()
	r.liquidation = append(r.liquidation, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addBookTicker(ev *models.BookTickerEvent) {
	r.mu.Lock()
	r.bookTicker = append(r.bookTicker, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addPartialDepth(ev *models.PartialDepthEvent) {
	r.mu.Lock()
	r.partialDepth = append(r.partialDepth, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addDiffDepth(ev *models.DiffDepthEvent) {
	r.mu.Lock()
	r.diffDepth = append(r.diffDepth, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAllMarkPrices(ev *models.AllMarkPricesEvent) {
	r.mu.Lock()
	r.allMarkPrices = append(r.allMarkPrices, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAllMiniTickers(ev *models.AllMiniTickersEvent) {
	r.mu.Lock()
	r.allMiniTickers = append(r.allMiniTickers, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAllTickers(ev *models.AllTickersEvent) {
	r.mu.Lock()
	r.allTickers = append(r.allTickers, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAllBookTickers(ev *models.AllBookTickersEvent) {
	r.mu.Lock()
	r.allBookTickers = append(r.allBookTickers, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAllLiquidations(ev *models.AllLiquidationsEvent) {
	r.mu.Lock()
	r.allLiquidations = append(r.allLiquidations, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addCompositeIndex(ev *models.CompositeIndexEvent) {
	r.mu.Lock()
	r.compositeIndex = append(r.compositeIndex, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAssetIndex(ev *models.AssetIndexEvent) {
	r.mu.Lock()
	r.assetIndex = append(r.assetIndex, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addAllAssetIndexes(ev *models.AllAssetIndexesEvent) {
	r.mu.Lock()
	r.allAssetIndexes = append(r.allAssetIndexes, ev)
	r.mu.Unlock()
}
func (r *umMarketEventRecorder) addContinuousKline(ev *models.ContinuousKlineEvent) {
	r.mu.Lock()
	r.continuousKline = append(r.continuousKline, ev)
	r.mu.Unlock()
}
...
func (r *umMarketEventRecorder) count(key string) int {
	...
	case "allMarkPrices":
		return len(r.allMarkPrices)
	case "allMiniTickers":
		return len(r.allMiniTickers)
	case "allTickers":
		return len(r.allTickers)
	case "allBookTickers":
		return len(r.allBookTickers)
	case "allLiquidations":
		return len(r.allLiquidations)
	case "compositeIndex":
		return len(r.compositeIndex)
	case "assetIndex":
		return len(r.assetIndex)
	case "allAssetIndexes":
		return len(r.allAssetIndexes)
	case "continuousKline":
		return len(r.continuousKline)
	default:
		return 0
	}
}
...
// TestFullIntegrationSuite_Market covers request/response + event handlers for MarketStreamChannel
func TestFullIntegrationSuite_Market(t *testing.T) {
	...
	market.HandleAllLiquidationsEvent(func(ctx context.Context, ev *models.AllLiquidationsEvent) error {
	...
	market.HandleAllAssetIndexesEvent(func(ctx context.Context, ev *models.AllAssetIndexesEvent) error {
		rec.addAllAssetIndexes(ev)
		return nil
	})
	market.HandleContinuousKlineEvent(func(ctx context.Context, ev *models.ContinuousKlineEvent) error {
	...
	t.Run("Request_ListSubscriptions", func(t *testing.T) {
		...
		if err := market.ListSubscriptions(context.Background(), &models.ListSubscriptionsRequest{Id: lid}, &listCb); err != nil {
	...
	t.Run("Request_SetProperty", func(t *testing.T) {
		// Use MarketStream setProperty on active connection (do not use combined here)
		pid := newMessageID()
		...
		setCb := func(ctx context.Context, resp *models.SetPropertyResponse) error {
			...
			if resp.Id.String() != pid.String() {
				t.Errorf("setProperty id mismatch: want %s got %s", pid.String(), resp.Id.String())
			}
			gotSet = resp
		...
		if err := market.SetProperty(spCtx, setReq, &setCb); err != nil {
			le := strings.ToLower(err.Error())
			if strings.Contains(le, "deadline") {
				t.Logf("setProperty timeout (acceptable): %v", err)
			} else {
	...
	t.Run("Request_GetProperty", func(t *testing.T) {
		// Query property on MarketStream (not combined)
		gid := newMessageID()
		...
		getCb := func(ctx context.Context, resp *models.GetPropertyResponse) error {
			...
			if resp.Id.String() != gid.String() {
				t.Errorf("getProperty id mismatch: want %s got %s", gid.String(), resp.Id.String())
			}
			gotGet = resp
	...
	t.Run("AggregateTradeEvent", func(t *testing.T) {
		...
		if cnt > 0 {
			...
			if !strings.EqualFold(ev.Symbol, symUpper) {
				t.Logf("unexpected symbol: %s", ev.Symbol)
			}
	...
	t.Run("AssetIndexEvent", func(t *testing.T) {
		...
		if cnt == 0 {
			t.Logf("no assetIndex event received (acceptable)")
		}
	})
	...
	t.Run("ContinuousKlineEvent", func(t *testing.T) {
		...
		if cnt == 0 {
			t.Logf("no continuousKline event received (acceptable)")
		}
	})
}
