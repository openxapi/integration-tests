...
type SharedClientManager struct {
	clients   map[string]*umfuturesws.Client
	mutex     sync.RWMutex
	cleanupFn func()
}
...
func setupClient(config TestConfig) (*umfuturesws.Client, error) {
...
func initSharedClients() {
	once.Do(func() {
		sharedClients = &SharedClientManager{
			clients: make(map[string]*umfuturesws.Client),
		}
...
func getOrCreateSharedClient(t *testing.T, config TestConfig) *umfuturesws.Client {
	initSharedClients()
...
func testEndpoint(t *testing.T, config TestConfig, testName string, testFunc func(*umfuturesws.Client, TestConfig) error) {
	testEndpointWithTimeout(t, config, testName, testFunc, 10*time.Second)
}

// Test function template with configurable timeout
func testEndpointWithTimeout(t *testing.T, config TestConfig, testName string, testFunc func(*umfuturesws.Client, TestConfig) error, timeout time.Duration) {
	t.Helper()
...
func getCurrentPrice(client *umfuturesws.Client, symbol string) (float64, error) {
