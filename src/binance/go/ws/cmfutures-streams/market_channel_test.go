...
func (r *cmMarketEventRecorder) addAgg(v *models.AggregateTradeEvent) {
	r.mu.Lock()
	r.agg = append(r.agg, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addMark(v *models.MarkPriceEvent) {
	r.mu.Lock()
	r.mark = append(r.mark, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addIndex(v *models.IndexPriceEvent) {
	r.mu.Lock()
	r.index = append(r.index, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addKline(v *models.KlineEvent) {
	r.mu.Lock()
	r.kline = append(r.kline, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addContK(v *models.ContinuousKlineEvent) {
	r.mu.Lock()
	r.contKline = append(r.contKline, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addIndexK(v *models.IndexKlineEvent) {
	r.mu.Lock()
	r.indexKline = append(r.indexKline, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addMarkK(v *models.MarkPriceKlineEvent) {
	r.mu.Lock()
	r.markKline = append(r.markKline, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addTicker(v *models.TickerEvent) {
	r.mu.Lock()
	r.ticker = append(r.ticker, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addMiniTicker(v *models.MiniTickerEvent) {
	r.mu.Lock()
	r.miniTicker = append(r.miniTicker, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addBookTicker(v *models.BookTickerEvent) {
	r.mu.Lock()
	r.bookTicker = append(r.bookTicker, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addPartialDepth(v *models.PartialDepthEvent) {
	r.mu.Lock()
	r.partialDepth = append(r.partialDepth, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addDiffDepth(v *models.DiffDepthEvent) {
	r.mu.Lock()
	r.diffDepth = append(r.diffDepth, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addLiquid(v *models.LiquidationEvent) {
	r.mu.Lock()
	r.liquidation = append(r.liquidation, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addAllMark(v *models.AllMarkPricesEvent) {
	r.mu.Lock()
	r.allMarkPrices = append(r.allMarkPrices, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addAllMini(v *models.AllMiniTickersEvent) {
	r.mu.Lock()
	r.allMiniTickers = append(r.allMiniTickers, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addAllTick(v *models.AllTickersEvent) {
	r.mu.Lock()
	r.allTickers = append(r.allTickers, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addAllBook(v *models.AllBookTickersEvent) {
	r.mu.Lock()
	r.allBookTickers = append(r.allBookTickers, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addAllLiquid(v *models.AllLiquidationsEvent) {
	r.mu.Lock()
	r.allLiquid = append(r.allLiquid, v)
	r.mu.Unlock()
}
func (r *cmMarketEventRecorder) addContract(v *models.ContractInfoEvent) {
	r.mu.Lock()
	r.contractInfo = append(r.contractInfo, v)
	r.mu.Unlock()
}
...
func (r *cmMarketEventRecorder) count(key string) int {
	...
	case "continuousKline":
		return len(r.contKline)
	case "indexKline":
		return len(r.indexKline)
	case "markPriceKline":
		return len(r.markKline)
	case "ticker":
		return len(r.ticker)
	case "miniTicker":
	...
	case "allLiquidations":
		return len(r.allLiquid)
	case "contractInfo":
		return len(r.contractInfo)
	default:
		return 0
	}
}
...
func TestFullIntegrationSuite_Market(t *testing.T) {
	...
	t.Run("Request_Subscribe", func(t *testing.T) {
		...
		if err := market.Subscribe(context.Background(), &models.SubscribeRequest{Id: sid, Params: []string{markPath, aggPath}}, &cb); err != nil {
		...
		_ = market.Unsubscribe(context.Background(), &models.UnsubscribeRequest{Id: newRequestID(), Params: []string{markPath}}, &ucb)
	})
	...
	t.Run("Request_ListSubscriptions", func(t *testing.T) {
		...
		_ = market.Subscribe(context.Background(), &models.SubscribeRequest{Id: newRequestID(), Params: []string{path}}, &subCb)
		lid := newRequestID()
		done := make(chan struct{}, 1)
		var got *models.ListSubscriptionsResponse
		var listErr error
		lcb := func(ctx context.Context, resp *models.ListSubscriptionsResponse, err error) error {
			if err != nil {
				listErr = err
				select {
				case done <- struct{}{}:
		...
		if err := market.ListSubscriptions(context.Background(), &models.ListSubscriptionsRequest{Id: lid}, &lcb); err != nil {
		...
		if listErr != nil {
			t.Fatalf("listSubscriptions handler returned error: %v", listErr)
		}
		if got == nil {
		...
		_ = market.Unsubscribe(context.Background(), &models.UnsubscribeRequest{Id: newRequestID(), Params: []string{path}}, &ucb)
	})
	...
	t.Run("Request_GetProperty", func(t *testing.T) {
		...
		if err := market.GetProperty(context.Background(), req, &getCb); err != nil {
