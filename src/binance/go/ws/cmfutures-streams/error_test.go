...
func TestErrorHandling(t *testing.T) {
	...
	validStream := "btcusd_perp@aggTrade"
...
func TestInvalidStreamNames(t *testing.T) {
	...
	// Use a single shared client and manage connection state properly
	client, isDedicated := setupTestClient(t)
	if isDedicated {
		defer client.Disconnect()
	}
	...
	invalidStreams := []string{
		...
		"btcusd_perp@invalidstream",
		"@aggTrade",
		"btcusd_perp@",
		"",
		"BTCUSDT@AGGTRADE", // Test case sensitivity
		"btcusd_perp@@aggTrade", // Double @
		"btcusd_perp@aggTrade@extra", // Extra parts
	}
...
func TestOperationsWithoutConnection(t *testing.T) {
	...
	err = client.Unsubscribe(ctx, []string{"btcusd_perp@aggTrade"})
	if err == nil {
...
func TestUnsubscribeNonexistentStream(t *testing.T) {
	...
	nonexistentStream := "btcusd_perp@aggTrade"
	...
	differentStream := "LINKUSD_PERP@aggTrade"
