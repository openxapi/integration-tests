...
func (r *cmUserDataEventRecorder) addExpired(v *models.ListenKeyExpiredEvent) {
	r.mu.Lock()
	r.expired = append(r.expired, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addAcctUpd(v *models.AccountUpdateEvent) {
	r.mu.Lock()
	r.acctUpd = append(r.acctUpd, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addMarginCal(v *models.MarginCallEvent) {
	r.mu.Lock()
	r.marginCal = append(r.marginCal, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addOrdUpd(v *models.OrderTradeUpdateEvent) {
	r.mu.Lock()
	r.ordUpd = append(r.ordUpd, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addCfgUpd(v *models.AccountConfigUpdateEvent) {
	r.mu.Lock()
	r.cfgUpd = append(r.cfgUpd, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addStratUpd(v *models.StrategyUpdateEvent) {
	r.mu.Lock()
	r.stratUpd = append(r.stratUpd, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addGridUpd(v *models.GridUpdateEvent) {
	r.mu.Lock()
	r.gridUpd = append(r.gridUpd, v)
	r.mu.Unlock()
}
func (r *cmUserDataEventRecorder) addError(v *models.ErrorMessage) {
	r.mu.Lock()
...
func createListenKeyTestnet(t *testing.T) string {
	...
	if err != nil || resp == nil || resp.ListenKey == nil || *resp.ListenKey == "" {
		t.Fatalf("failed to create listenKey: %v", err)
	}
...
func cancelAllOpenOrders(t *testing.T) {
	...
	for sym := range syms {
		...
		if err != nil {
			t.Logf("cancelAllOpenOrders: cancel %s failed: %v", sym, err)
		}
	}
}
...
func TestFullIntegrationSuite_UserData(t *testing.T) {
	...
	t.Run("Request_Start", func(t *testing.T) {
		if isTestnet {
			t.Skip("Skip userDataStream.start on testnet (unsupported)")
		}
		sid := newRequestID()
		...
		if k := os.Getenv("BINANCE_API_KEY"); k != "" {
			req.Params.ApiKey = k
		}
		done := make(chan struct{}, 1)
		startCb := func(ctx context.Context, resp *models.UserDataStreamStartResponse, err error) error {
			if err != nil {
				var apiErr *models.ErrorMessage
				if errors.As(err, &apiErr) {
					rec.addError(apiErr)
					logJSON(t, "userData.start.error", apiErr)
				} else {
					t.Logf("start handler error: %v", err)
				}
				select {
				case done <- struct{}{}:
				default:
		...
		if err := ch.Start(context.Background(), req, &startCb); err != nil {
			t.Logf("start err (acceptable): %v", err)
		}
		select {
		case <-done:
		case <-time.After(5 * time.Second):
			t.Logf("timeout waiting start response (acceptable)")
		}
		...
		if got > before {
			...
			if em == nil {
				t.Fatalf("nil error captured")
			}
			if !msgIDEqual(em.Id, sid) {
				t.Logf("error id mismatch: want=%s got=%s", sid.String(), em.Id.String())
			}
			if em.ErrorPayload.Code == 0 || strings.TrimSpace(em.ErrorPayload.Msg) == "" {
	...
	t.Run("Request_Ping", func(t *testing.T) {
		if isTestnet {
			t.Skip("Skip userDataStream.ping on testnet (unsupported)")
		}
		pid := newRequestID()
		done := make(chan struct{}, 1)
		var pingErr error
		cb := func(ctx context.Context, resp *models.UserDataStreamPingResponse, err error) error {
			if err != nil {
				pingErr = err
				logJSON(t, "userData.ping.error", err)
				select {
				case done <- struct{}{}:
				default:
				}
		...
		if err := ch.Ping(context.Background(), &models.UserDataStreamPingRequest{Id: pid}, &cb); err != nil {
			t.Logf("ping err (acceptable): %v", err)
		}
		select {
		case <-done:
		case <-time.After(5 * time.Second):
			t.Logf("timeout waiting ping response (acceptable)")
		}
		if pingErr != nil {
			t.Logf("ping handler error (acceptable): %v", pingErr)
		}
	})

	t.Run("Request_Stop", func(t *testing.T) {
		if isTestnet {
			t.Skip("Skip userDataStream.stop on testnet (unsupported)")
		}
		rid := newRequestID()
		before := rec.count("error")
		...
		stopCb := func(ctx context.Context, resp *models.UserDataStreamStopResponse, err error) error {
			if err != nil {
				var apiErr *models.ErrorMessage
				if errors.As(err, &apiErr) {
					rec.addError(apiErr)
					logJSON(t, "userData.stop.error", apiErr)
				} else {
					t.Logf("stop handler error: %v", err)
				}
				select {
				case done <- struct{}{}:
				default:
			...
			if resp != nil && !msgIDEqual(resp.Id, rid) {
				t.Logf("stop response id mismatch: want=%s got=%s", rid.String(), resp.Id.String())
			}
			if resp != nil {
		...
		if err := ch.Stop(context.Background(), req, &stopCb); err != nil {
			t.Logf("stop err (acceptable): %v", err)
		}
		select {
		case <-done:
		case <-time.After(5 * time.Second):
			t.Logf("timeout waiting stop response (acceptable)")
		}
		_ = rec.waitForMin("error", before+1, 5*time.Second)
		got := rec.count("error")
	...
	t.Run("OrderTradeUpdateEvent", func(t *testing.T) {
		...
		if cnt > 0 {
			...
			if ev.EventTime <= 0 || ev.TransactionTime <= 0 {
				t.Errorf("invalid timestamps: E=%d T=%d", ev.EventTime, ev.TransactionTime)
			}
			if ev.OrderDetails.Symbol == "" {
				t.Errorf("empty order symbol")
			}
			if ev.OrderDetails.OrderID == 0 {
				t.Logf("order id missing or zero")
			}
		}
	})
	...
	t.Run("AccountConfigUpdateEvent", func(t *testing.T) {
		...
		if cnt > before {
			...
			if ev.EventType != "ACCOUNT_CONFIG_UPDATE" {
				t.Errorf("want e=ACCOUNT_CONFIG_UPDATE got %s", ev.EventType)
			}
			if ev.EventTime <= 0 || ev.TransactionTime <= 0 {
				t.Logf("config update not recent: E=%d T=%d", ev.EventTime, ev.TransactionTime)
			}
			// If symbol-level config exists, assert leverage sensible
			if ev.AccountConfigurationForTradePair.Symbol != "" {
				if ev.AccountConfigurationForTradePair.Leverage <= 0 {
					t.Errorf("invalid leverage in config update: %d", ev.AccountConfigurationForTradePair.Leverage)
				}
			}
		}
	})
	...
	t.Run("AccountUpdateEvent", func(t *testing.T) {
		...
		if cnt > 0 {
			...
			if ev.EventType != "ACCOUNT_UPDATE" {
				t.Errorf("want e=ACCOUNT_UPDATE got %s", ev.EventType)
			}
			if ev.EventTime <= 0 || ev.TransactionTime <= 0 {
				t.Logf("account update timestamps not set: E=%d T=%d", ev.EventTime, ev.TransactionTime)
			}
