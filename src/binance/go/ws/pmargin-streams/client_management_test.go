...
func TestClient_WithOptionsAndAuth(t *testing.T) {
	opts := &pmarginstreams.ClientOptions{HandlerWorkers: 2}
	client := pmarginstreams.NewClientWithOptions(opts)
	if client == nil {
	...
	auth := pmarginstreams.NewAuth("test-key")
	auth.SetSecretKey("test-secret")
	clientWithAuth := pmarginstreams.NewClientWithAuth(auth)
	...
	// Safe to call StopReadLoop/Wait without a connection
...
func TestClient_ServerLifecycle(t *testing.T) {
	...
	err := client.AddServer("testnet", "wss://example.test/pm", "Test Server", "Test description")
	if err != nil {
	...
	} else if srv.URL != "wss://example.test/pm" {
		t.Errorf("GetServer URL mismatch: %s", srv.URL)
	}

	err = client.AddOrUpdateServer("testnet", "wss://example.test/pm/ws", "Test Server (updated)", "Updated description")
	if err != nil {
	...
	if got := client.GetCurrentURL(); !strings.HasPrefix(got, "wss://example.test/pm") {
		t.Errorf("unexpected active URL: %s", got)
	}

	err = client.UpdateServer("testnet", "wss://example.test/pm/ws", "Test Server (final)", "Final description")
	if err != nil {
		t.Fatalf("UpdateServer failed: %v", err)
	}

	// Removing active server should fail
	if err := client.RemoveServer("testnet"); err == nil {
		t.Fatalf("RemoveServer succeeded for active server")
	}

	// Switch back to default active server and remove the test server
	if defaultActive != nil {
...
func TestServerManagerLifecycle(t *testing.T) {
	sm := pmarginstreams.NewServerManager()
	primary := &pmarginstreams.ServerInfo{
	...
	secondary := &pmarginstreams.ServerInfo{
	...
	if err := sm.UpdateServer("secondary", &pmarginstreams.ServerInfo{
		URL:         "wss://secondary.example/pm/ws/{listenKey}",
		Protocol:    "wss",
		Host:        "secondary.example",
		Pathname:    "/pm/ws/{listenKey}",
