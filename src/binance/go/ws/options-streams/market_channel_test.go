...
func TestFullIntegrationSuite_Market(t *testing.T) {
    ...
    market.HandleTickerByUnderlyingEvent(func(ctx context.Context, ev *models.TickerByUnderlyingEvent) error { rec.addTickerByUnderlying(ev); return nil })
    ...
    market.HandleOpenInterestEvent(func(ctx context.Context, ev *models.OpenInterestEvent) error { rec.addOpenInterest(ev); return nil })
    {
        ...
        ccancel()
        defer func(){ dctx,dcancel:=context.WithTimeout(context.Background(),5*time.Second); _=market.Disconnect(dctx); dcancel() }()
    }
    ...
    t.Run("Request_ListSubscriptions", func(t *testing.T) {
        ...
        if gotList == nil { t.Fatalf("did not capture listSubscriptions response") }
        // Cleanup
    ...
    t.Run("Request_GetProperty", func(t *testing.T) {
        ...
        if gotGet == nil { t.Fatalf("did not capture getProperty response") }
        // Do not unsubscribe the base index stream here to keep the connection stable across suite
    })
    ...
    t.Run("Request_Unsubscribe", func(t *testing.T) {
        ...
        unsubCb := func(ctx context.Context, resp *models.UnsubscriptionResponse) error {
            if resp == nil { t.Errorf("nil unsubscribe response"); return nil }
            if resp.Id != uid { t.Errorf("unsubscribe id mismatch: want %d got %d", uid, resp.Id) }
            logJSON(t, "unsubscribe.response", resp)
            select { case unsubDone <- struct{}{}: default: }
            return nil
        }
        ...
        case <-time.After(12 * time.Second):
            ...
            lsCb := func(ctx context.Context, resp *models.ListSubscriptionsResponse) error {
                if resp != nil && resp.Id == lid {
                    listed = resp.Result
                    logJSON(t, "listSubscriptions.response", resp)
                    select { case lsDone <- struct{}{}: default: }
                }
                return nil
            }
    ...
    t.Run("IndexPriceEvent", func(t *testing.T) {
        ...
        if evs := rec.getIndex(); len(evs) > 0 {
            ev := evs[len(evs)-1]
            if ev.EventType != "index" { t.Errorf("want e=index got %s", ev.EventType) }
            if ev.UnderlyingSymbol == "" || !strings.EqualFold(ev.UnderlyingSymbol, underlyingUpper + "USDT") { t.Errorf("unexpected symbol: %s", ev.UnderlyingSymbol) }
            ...
            if restValidationEnabled() {
                ...
                } else { t.Logf("REST index fetch failed: %v", err) }
            }
        } else {
    ...
    t.Run("MarkPriceEvent", func(t *testing.T) {
        ...
        if evs := rec.getMark(); len(evs) > 0 && evs[len(evs)-1] != nil {
            ev := evs[len(evs)-1]
            if ev == nil { t.Fatalf("nil markPrice event") }
            if len(*ev) == 0 { t.Errorf("empty markPrice array") }
            for i, it := range *ev {
                if it.EventType != "markPrice" { t.Errorf("want e=markPrice got %s", it.EventType) }
                ...
                if i == 0 && restValidationEnabled() {
                    ...
                    } else if err != nil { t.Logf("REST mark fetch failed: %v", err) }
                }
            }
        } else {
    ...
    t.Run("KlineEvent", func(t *testing.T) {
        ...
        if err != nil { t.Skipf("no active symbol: %v", err) }
        ...
        if evs := rec.getKline(); len(evs) > 0 {
            ...
            if ev.KlineData.Interval != interval { t.Logf("interval mismatch (got %s)", ev.KlineData.Interval) }
    ...
    t.Run("TickerEvent", func(t *testing.T) {
        ...
        if evs := rec.getTicker(); len(evs) > 0 {
            ...
            if restValidationEnabled() {
                ...
                } else if err != nil { t.Logf("REST ticker fetch failed: %v", err) }
            }
    ...
    t.Run("TickerByUnderlyingEvent", func(t *testing.T) {
        ...
        if err != nil || len(exps) == 0 { t.Skipf("no expiration dates: %v", err) }
        ...
        if err != nil { t.Fatalf("build ticker-by-underlying stream: %v", err) }
        // Subscribe on the existing connection (global handlers will record events)
        ...
        if evs := rec.getTickerByUnderlying(); len(evs) > 0 && evs[len(evs)-1] != nil {
            ev := evs[len(evs)-1]
            if len(*ev) == 0 { t.Errorf("empty tickerByUnderlying array") }
            it := (*ev)[0]
            if it.EventType != "24hrTicker" { t.Errorf("want e=24hrTicker got %s", it.EventType) }
            assertOptionSymbolFormat(t, it.OptionSymbol)
            if restValidationEnabled() {
    ...
    t.Run("TradeEvent", func(t *testing.T) {
        ...
        if err != nil { t.Fatalf("build trade stream: %v", err) }
        // Subscribe on the existing connection (global handlers will record events)
        ...
        if evs := rec.getTrade(); len(evs) > 0 {
            ev := evs[len(evs)-1]
            if ev.EventType != "trade" { t.Errorf("want e=trade got %s", ev.EventType) }
            ...
            if ev.TradeID <= 0 { t.Errorf("invalid trade id: %d", ev.TradeID) }
            if ev.TradeCompletedTimestamp <= 0 { t.Errorf("invalid trade ts: %d", ev.TradeCompletedTimestamp) }
        } else {
    ...
    t.Run("PartialDepthEvent", func(t *testing.T) {
        ...
        if evs := rec.getPartialDepth(); len(evs) > 0 {
            ev := evs[len(evs)-1]
            if ev.EventType != "depth" { t.Errorf("want e=depth got %s", ev.EventType) }
            assertNonEmpty(t, ev.OptionSymbol, "symbol")
            assertOptionSymbolFormat(t, ev.OptionSymbol)
            if len(ev.BuyOrders) > 0 {
                b := ev.BuyOrders[0]
                if len(b) >= 2 { _ = tryParseFloat(t, b[0], "bidPrice"); _ = tryParseFloat(t, b[1], "bidQty") }
            }
            if len(ev.SellOrders) > 0 {
                a := ev.SellOrders[0]
                if len(a) >= 2 { _ = tryParseFloat(t, a[0], "askPrice"); _ = tryParseFloat(t, a[1], "askQty") }
            }
            if ev.UpdateIDInEvent <= ev.PreviousUpdateID { t.Logf("non-increasing update ids: u=%d pu=%d", ev.UpdateIDInEvent, ev.PreviousUpdateID) }
        } else {
    ...
    t.Run("NewSymbolInfoEvent", func(t *testing.T) {
        ...
        if err != nil { t.Fatalf("build newSymbolInfo stream: %v", err) }
        // Subscribe on the existing connection (global handlers will record events)
        ...
        if evs := rec.getNewSymbolInfo(); len(evs) > 0 {
            ev := evs[len(evs)-1]
            if ev.EventType != "OPTION_PAIR" { t.Errorf("want e=OPTION_PAIR got %s", ev.EventType) }
            ...
            if restValidationEnabled() {
                ...
                if syms, err := getActiveOptionsSymbols(u); err == nil {
                    found := false
                    for _, s := range syms { if s == ev.TradingPairName { found = true; break } }
                    if !found { t.Logf("symbol from newSymbolInfo not found in exchange info list (may be brand new): %s", ev.TradingPairName) }
                }
            }
        } else {
    ...
    t.Run("OpenInterestEvent", func(t *testing.T) {
        ...
        if err != nil { t.Fatalf("build openInterest stream: %v", err) }
        // Subscribe on the existing connection (global handlers will record events)
        ...
        if evs := rec.getOpenInterest(); len(evs) > 0 {
            ...
            if ev == nil || len(*ev) == 0 {
                ...
                for _, it := range *ev {
                    if it.EventType != "openInterest" { t.Errorf("want e=openInterest got %s", it.EventType) }
                    assertOptionSymbolFormat(t, it.OptionSymbol)
...
func contains(arr []string, s string) bool {
    for _, v := range arr { if v == s { return true } }
    return false
}
...
func (r *marketEventRecorder) addIndex(ev *models.IndexPriceEvent) { r.mu.Lock(); r.index = append(r.index, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addMark(ev *models.MarkPriceEvent)   { r.mu.Lock(); r.mark = append(r.mark, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addKline(ev *models.KlineEvent)      { r.mu.Lock(); r.kline = append(r.kline, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addTicker(ev *models.TickerEvent)    { r.mu.Lock(); r.ticker = append(r.ticker, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addTickerByUnderlying(ev *models.TickerByUnderlyingEvent){ r.mu.Lock(); r.tickerBy = append(r.tickerBy, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addTrade(ev *models.TradeEvent)      { r.mu.Lock(); r.trade = append(r.trade, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addPartialDepth(ev *models.PartialDepthEvent){ r.mu.Lock(); r.partialDepth = append(r.partialDepth, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addNewSymbolInfo(ev *models.NewSymbolInfoEvent){ r.mu.Lock(); r.newSymbolInfo = append(r.newSymbolInfo, ev); r.mu.Unlock() }
func (r *marketEventRecorder) addOpenInterest(ev *models.OpenInterestEvent){
    ...
    if ev == nil || len(*ev) == 0 { return }
    if (*ev)[0].EventType != "openInterest" { return }
    r.mu.Lock(); r.openInterest = append(r.openInterest, ev); r.mu.Unlock()
}

func (r *marketEventRecorder) getIndex() []*models.IndexPriceEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.IndexPriceEvent, len(r.index)); copy(out, r.index); return out }
func (r *marketEventRecorder) getMark() []*models.MarkPriceEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.MarkPriceEvent, len(r.mark)); copy(out, r.mark); return out }
func (r *marketEventRecorder) getKline() []*models.KlineEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.KlineEvent, len(r.kline)); copy(out, r.kline); return out }
func (r *marketEventRecorder) getTicker() []*models.TickerEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.TickerEvent, len(r.ticker)); copy(out, r.ticker); return out }
func (r *marketEventRecorder) getTickerByUnderlying() []*models.TickerByUnderlyingEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.TickerByUnderlyingEvent, len(r.tickerBy)); copy(out, r.tickerBy); return out }
func (r *marketEventRecorder) getTrade() []*models.TradeEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.TradeEvent, len(r.trade)); copy(out, r.trade); return out }
func (r *marketEventRecorder) getPartialDepth() []*models.PartialDepthEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.PartialDepthEvent, len(r.partialDepth)); copy(out, r.partialDepth); return out }
func (r *marketEventRecorder) getNewSymbolInfo() []*models.NewSymbolInfoEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.NewSymbolInfoEvent, len(r.newSymbolInfo)); copy(out, r.newSymbolInfo); return out }
func (r *marketEventRecorder) getOpenInterest() []*models.OpenInterestEvent { r.mu.RLock(); defer r.mu.RUnlock(); out := make([]*models.OpenInterestEvent, len(r.openInterest)); copy(out, r.openInterest); return out }
...
func (r *marketEventRecorder) count(key string) int {
    ...
    defer r.mu.RUnlock()
    switch key {
    case "index": return len(r.index)
    case "markPrice": return len(r.mark)
    case "kline": return len(r.kline)
    case "ticker": return len(r.ticker)
    case "tickerByUnderlying": return len(r.tickerBy)
    case "trade": return len(r.trade)
    case "partialDepth": return len(r.partialDepth)
    case "newSymbolInfo": return len(r.newSymbolInfo)
    case "openInterest": return len(r.openInterest)
    default: return 0
    }
}
